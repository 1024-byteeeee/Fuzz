...
package top.byteeeee.kkk.translations;
...
public class Translator {
    ...
    public BaseText tr(String key, Object... args) {
        String translationKey = KKKModClient.MOD_ID + "." + this.translationPath + "." + key;
        return Messenger.tr(translationKey, args);
    }

    public String getFuncNameTrKey(String name) {
        return String.format("kkk.settings.%s.name", name);
    }

    public String getFuncDescTrKey(String name) {
        return String.format("kkk.settings.%s.desc", name);
    }
}
