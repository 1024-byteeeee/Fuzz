...
public abstract class EntityMixin {
    ...
    private boolean noUpdate(boolean original) {
        ...
        if (KKKSettings.letFluidInteractLikeAir && entity.equals(ClientUtil.getCurrentPlayer()) && !entity.isOnFire()) {
            return false;
        } else {
            return original;
        }
    ...
    private boolean setSwimming(Entity entity, int index, boolean value) {
        return !KKKSettings.letFluidInteractLikeAir && entity.equals(ClientUtil.getCurrentPlayer());
    }

    @ModifyReturnValue(method = "getFluidHeight", at = @At("RETURN"))
    private double getFluidHeight(double original) {
        Entity entity = (Entity) (Object) this;
        if (KKKSettings.letFluidInteractLikeAir && entity.equals(ClientUtil.getCurrentPlayer())) {
            return 0.114514D;
        } else {
            return original;
        }
