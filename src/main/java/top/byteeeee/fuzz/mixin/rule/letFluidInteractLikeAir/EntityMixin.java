...
public abstract class EntityMixin {
    ...
    private boolean isInWater(boolean original) {
        ...
        if (FuzzSettings.letFluidInteractLikeAir && ClientUtil.isSelf(entity) && !entity.isOnFire()) {
            return false;
        } else {
            return original;
        }
    ...
    private boolean setSwimming(Entity entity, int index, boolean value) {
        return !FuzzSettings.letFluidInteractLikeAir && ClientUtil.isSelf(entity);
    }

    @ModifyReturnValue(method = "getFluidHeight", at = @At("RETURN"))
    private double getFluidHeight(double original) {
        Entity entity = (Entity) (Object) this;
        if (FuzzSettings.letFluidInteractLikeAir && ClientUtil.isSelf(entity)) {
            return 0.114514D;
        } else {
            return original;
        }
